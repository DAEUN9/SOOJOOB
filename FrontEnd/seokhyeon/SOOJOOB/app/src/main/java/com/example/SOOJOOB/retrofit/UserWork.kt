...
class UserWork() {
    fun work(completion : (statusCode:Int, username:String?, trash:Int?, exp:Double?, badge:Int?, id:Int?) -> Unit) {
            ...
            .enqueue(object : retrofit2.Callback<UserResponseBody> {
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        val id = response.body()?.data?.userRecord?.id
                        ...
                        completion(response.code(), "$username", trash, exp, badge, id)
                    }
                    else {
