...
class MapsActivity : AppCompatActivity(), GoogleMap.OnMyLocationButtonClickListener,
    ...
    // 바텀 바 크기 변경
    private lateinit var bottom_bar: Button
    private lateinit var scroll1: FrameLayout
    private lateinit var bottom_View: LinearLayout
    // 현재 위치
...
//        locationRequest.interval=10000 // 위치를 갱신하는데 필요한 시간 <밀리초 기준>
        ...
        locationRequest.fastestInterval=3000 // 다른 앱에서 위치를 갱신했을 때 그 정보를 가져오는 시간 <밀리초 기준>
...
//            addItems()
        ...
        bottom_bar.setOnClickListener{
            if(bottomflag) {
                scroll1.layoutParams.height = 1700
                bottom_View.layoutParams.height = 200
                println(scroll1.height)
                println(bottom_View.height)
            } else{
                scroll1.layoutParams.height = 1200
                bottom_View.layoutParams.height = 700
                println(scroll1.height)
                println(bottom_View.height)
            }
            /*
